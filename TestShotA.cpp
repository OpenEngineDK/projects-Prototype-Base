...
namespace OpenEngine {
	namespace Prototype {
		TestShotA::TestShotA(Vector<3,float> from, Vector<3,float> to) {
			this->from = from;
			this->to = to;
		...
		TestShotA::~TestShotA() {
                ...
                void TestShotA::Initialize() {
                }
                void TestShotA::Deinitialize() {
                }
                bool TestShotA::IsTypeOf(const std::type_info& inf) {
                    return false;
                }
                void TestShotA::Process(const float dt, const float percent) {
                        Vector<3,float> p1 = from;
			Vector<3,float> p2 = to;
                        from += (p2 - p1)*shotSpeed*dt;
                        to += (p2 - p1)*shotSpeed*dt;
                }

		void TestShotA::Apply(IRenderingView* view) {
